...
class UnitRecoveryTracking extends Component {
    ...
    markUnitForTracking() {
        if (this.state.selectedUnit === -1)
            ...
            this.props.markUnitForTracking(this.state.selectedUnit, this.state.currentMealId, this.props.selectedKitchenRecovered, false, this.props.facilityMealNameArray[mealNameArrayIndex].name);
            this.setState({ selectedUnit: -1 })
        }
    }
    ...
    fetchUnits(kitchenId, mealNameId) {
        if (mealNameId !== -1 && this.props.facilityMealNameArray) {
			...
			this.props.fetchRecoveryUnits(kitchenId, mealNameId, this.props.facilityMealNameArray[mealNameArrayIndex].name);
            this.setState({ selectedUnit: -1 })
        }
    }
    ...
    onUndo(unit) {
        if (this.state.currentMealId !== -1 && this.props.facilityMealNameArray) {
			var mealNameArrayIndex = this.state.currentMealId === 1 ? 1 : this.state.currentMealId === 3 ? 2 : 3;
            this.props.undoUnitTracking(unit, this.state.currentMealId, this.props.selectedKitchenRecovered, false, this.props.facilityMealNameArray[mealNameArrayIndex].name)
            this.setState({ selectedUnit: -1 })
        }
    }
